...
def scrape_en_save(koers_naam, url):
    try:
        ...
        for row in rows:
            ...
            for link in links:
                href = link['href']
                if "rider/" in href and not rider: rider = link.get_text(strip=True)
                elif "team/" in href and not team: team = link.get_text(strip=True)
...
# --- MAIN APP ---
pagina = st.sidebar.radio("Menu", ["Klassement", "Uitslag per Koers", "Teams & Scores", "Mijn Team", "Captains Kiezen", "Admin: Beheer"])
...
# 2. UITSLAG PER KOERS
    ...
    if not u_all.empty:
        ...
        # --- ALLES HIERONDER MOET OOK INGESPRONGEN STAAN ---
        st.divider()
        ...
        if data:
            ...
            # We gebruiken st.dataframe in plaats van st.table voor meer controle
            st.dataframe(
                df_koers_punten,
                column_config={
                    ...
                    "Scorende Renners": st.column_config.TextColumn(
                        "Scorende Renners",
                        width="large",  # Geef deze kolom de meeste ruimte
                        help="Renners die punten hebben gescoord of gefinisht zijn"
                    ),
                },
                hide_index=False, # Zet op True als je de 1, 2, 3 index niet wilt zien
                use_container_width=True
            )
        else:
        ...
        if st.button("Start Scraper voor alle koersen"):
            ...
            if not df_k.empty:
                ...
                for index, r in df_k.iterrows():
                    ...
                    if ok:
                        st.write(f"✅ {k_naam}: {msg}")
                    else:
                        st.write(f"❌ {k_naam}: {msg}")
                
                status_placeholder.success("Alle koersen zijn verwerkt!")
                time.sleep(2)
                st.rerun()
            else:
                st.error("Geen koersen gevonden in tabblad 'koersen'.")

    # --- DEZE LIJN EN DE KNOP HIERONDER MOETEN GELIJK STAAN MET DE 'with' BLOKKEN ---
    st.divider()
    
    st.subheader("🗑️ Data opschonen")
    if st.button("Verwijder ALLE uitslagen"):
        # Maak een leeg DataFrame met de juiste koppen
        standard_cols = ['koers_naam', 'rank', 'rider', 'team']
        ws_u = sh.worksheet("uitslagen")
        ws_u.clear()
        
        # Schrijf alleen de koppen terug
        ws_u.update([standard_cols])
        
        # Cache legen zodat de app direct 'leeg' lijkt
        st.cache_data.clear()
        
        st.warning("Alle uitslagen zijn gewist!")
        time.sleep(1)
        st.rerun()